...
char isMandelbrot(double c_r,double c_i,int I_max){//verifies if the element belongs to the mandelbrot set
		...
		for(;counter < I_max && module(z_r,z_i) < 2;counter++)
			{
			double z_r_1= z_r*z_r-z_i*z_i;
			double z_i_1=2*z_r*z_i;
			
			double z_r_2=z_r_1*z_r-z_i_1*z_i;
			double z_i_2=z_r_1*z_i+z_i_1*z_r;
			
			double z_r_3=z_r_2*z_r-z_i_2*z_i;
			double z_i_3=z_r_2*z_i+z_i_2*z_r;
			
			double z_r_4=z_r_3*z_r-z_i_3*z_i;
			double z_i_4=z_r_3*z_i+z_i_3*z_r;

			double z_r_5=z_r_4*z_r-z_i_4*z_i+c_r;
			double z_i_5=z_r_4*z_i+z_i_4*z_r+c_i;
