...
#PBS -N schedule_collapse_strong_scalability_on_mandelbrot_scheduling
#PBS -l walltime=00:20:00
cd $HOME/FHPC_2019-2020/Workspace_3

VAR=results/time_execution_strong_mandelbrot_scheduling_collapse.csv
...
rm $VAR
 ...
 do
 	...
 	for (( i=0; i<=$NSHOTS; i=i+1 )) 
		...
		realtime=$( /usr/bin/time -f "buuuh %E" ./parallel_mandelbrot_schedule_collapse $N $N -2 -2 2 2 100 2>&1 | grep buuuh | cut -d' ' -f2) 
